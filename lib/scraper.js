...
Scraper.prototype.push = function(uri) {
    if (!this.queue) {
        ...
        this.queue.drain = this.queue.drain || function() {
            this.emit('finish');
        }.bind(this);
    }
...
Scraper.prototype.fetch = function(uri, callback) {
    console.log("Fetching %s with %s items left in the worker queue.", uri, this.queue.length());
    request({
        strictSSL: false,
        uri: uri
...
Scraper.prototype.store = function(s, callback) {
    ...
    _.delay(callback, 10); // throttles it a bit
}
