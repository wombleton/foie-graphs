...
Repo.prototype.getPath = function(date, body) {
    ...
    return util.format('%s/%s', date.getYear(), body);
};

Repo.prototype.read = function(path, callback) {
    repo.read('data', path, function(err, content) {
        if (err) {
            if (err === 'not found') {
                callback(null, null);
            } else {
...
Repo.prototype.changed = function(date, body, s, callback) {
...
Repo.prototype.update = function(path, data, callback) {
    ...
    this.changed(path, s, function(err, changed) {
        ...
        if (changed) {
            repo.write('data', path, s, 'Update.', callback);
        } else {
            callback();
        }
    });
